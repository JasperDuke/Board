...
const buildIssueStats = (
  ...
  const initialStats = Object.fromEntries(
    projectIds.map((id) => [
      id,
      { openIssues: 0, inProgressIssues: 0, blockedIssues: 0 },
    ]),
  );
...
export async function getMyProjects(userId: string): Promise<ProjectSummary[]> {
  try {
    ...
    const normalizedIssueCounts: IssueStatusCount[] = issueCounts.map(
      (count) => ({
    ...
    const projects: ProjectSummary[] = memberships.map((membership) => {
      ...
      const standupEnabled = Boolean(
        project.settings?.standupWindowStart &&
        project.settings?.standupWindowEnd,
      );
